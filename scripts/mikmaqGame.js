 ...
 *         Raish Raj Joshi- Functions for dragging and dropping, documentation of the entire file, deciding correct/incorrect ans,sizing.
...
const SERVER_URL = "http://ugdev.cs.smu.ca:3055";
...
//let inAnswer = 0; //total incorrect answers
...
function post() {
  ...
  const values = { cAnswer: oldValue, inAnswer: totalAnswer };

  // attempt to POST obj to endpoint http://ugdev.cs.smu.ca:3055/myPost
  // if (the middleware for this endpoint ran without error)
  //   call successFn
  // else
  //   call errorFn
  $.post(SERVER_URL + "/myPost", values, successPost).fail(errorFn);
}
...
*/
function get() {
  // attempt to GET a JSON object from endpoint http://ugdev.cs.smu.ca:3055/myGet
  // if (the middleware for this endpoint ran without error)
  //   call successFn
  // else
  //   call errorFn
 ...
 */
function successPost(returnedData) {
  console.log(returnedData);
}

//swaraj
function successGet(returnedData) {
  let oldValue = returnedData.cAnswer;
  console.log("successGet : " + oldValue);
}
 ...
 *
 * Author: Raish Raj Joshi - Edited function to display value of variables
 *         Suyog Chitrakar - Wrote initial function
 *         Nicolas Sabbagha - Updated code
 */
function clickScore() {
...
function insertSiowasiModal() {
  document.getElementById("sampleHide").style.display = "none";
  $("#resultModal").html(
    "<button id = 'siButton' class = 'siowasiButton' alt='siowasi'onclick='refreshPage();post()'>si'owa'si?</button>"
  );
}
/**
 ...
 *
 * Author: Swaraj Shrestha- Wrote initial function
 *         Raish Raj Joshi- funcition displayStarOrSun() with parameters
 */
 ...
 *
 * Author: Raish Raj Joshi- Wrote initial function, set draggable to false after drop
